    ...
    <el-form ref="form" :model="form"   :rules="rules" class="login-page">
      ...
      <el-form-item prop="username" >
        <el-input v-model="form.username"  clearable>
          <template #prefix>  <!-- 在 输入框最左侧 塞一个图标-->
...
export default {
  ...
  data() {
    return {
      ...
      form: {},     // 存储用户输入的用户名、密码、验证码
      rules: {      // Element Plus 的表单验证规则
        username: [
          {
            required: true,
            message: '请输入用户名',
        ...
        if (valid) {
          ...
          request.post("user/login", this.form).then(res => {
            if (res.code == 0) {
              ...
              sessionStorage.setItem("user",JSON.stringify(res.data))//缓存用户信息   res.data转换成字符串格式
